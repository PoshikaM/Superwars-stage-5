...
class Superwar {
  ...
  fight = () => {
    ...
    let finalStr = document.getElementById('score');
    finalStr.innerHTML = '1 - 0';
  ...
  calculateScore = () => {
    ...
    let settingWinner = this.updateWin();
    let score = this.score;
    this.players.map((player) => {
      if (player.type == 'hero') {
        player.win = settingWinner['hero'];
        score['hero'] += player.wins;
      } else {
        player.win = settingWinner['villain'];
